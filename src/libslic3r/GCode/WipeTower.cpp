...
WipeTower::ToolChangeResult WipeTower::tool_change(size_t tool, bool extrude_perimeter, bool first_toolchange_to_nonsoluble)
{
    ...
    if (tool != (unsigned int)-1){ 			// This is not the last change.
        int matl_temp = is_first_layer() ? m_filpar[tool].nozzle_temperature_initial_layer : m_filpar[tool].nozzle_temperature;
        writer.append(";" + GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Wipe_Tower_Start) + "\n");
        toolchange_Unload(writer, cleaning_box, m_filpar[m_current_tool].material,
                          (is_first_layer() ? m_filpar[m_current_tool].nozzle_temperature_initial_layer : m_filpar[m_current_tool].nozzle_temperature),
                          matl_temp);
        ...
        toolchange_Wipe(writer, cleaning_box, wipe_length, matl_temp);     // Wipe the newly loaded filament until the end of the assigned wipe area.
